...
public class BossAgent : Agent
{
    ...
    private void UpdateRunningSkill()
    {
        ...
        // 現在の瞬間的なパフォーマンスを計算
        // 生存(0.5) + 攻撃(0.3) + 回避(0.2) の比重を維持
        float survival = p.CurrentHealthRatio * 0.5f;
        // リアルタイムなので「現在の命中数」を期待値（例: 経過時間×2発）で割るなどの工夫
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        // ★ 修正ポイント：AIが弾幕の「バラけ方」を制御する (Continuous Actions を 12まで使用)
        // CA[8, 9] : 速度の振れ幅(Amp) と 周期(Freq)
        // CA[10, 11]: 角度の振れ幅(Amp) と 周期(Freq)
        ...
        // 密度の基準を spread/5 に引き上げ (全方位最大72発)
        ...
        if (DA[0] == 1 && fireCooldown <= 0 && currentEnergy >= totalCost)
        {
            ...
            for (int l = 0; l < layerCount; l++)
            {
                ...
                {
                    // ★ 修正ポイント：AIが選択した振幅と周期で「形」を作る (決定論的)
                    // 隣り合う弾(i)ごとに Sin/Cos で計算するため、ランダムではないが複雑な形になる
